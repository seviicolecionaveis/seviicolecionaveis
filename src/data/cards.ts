...
export type Finish = "Normal" | "Foil" | "Reverse Foil" | "Pokebola" | "Energia" | "Promo" | "Ímã";
...
export const EXTRA_COLLECTIONS = ["MEW - 151", "VIV - Voltagem Vívida", "ASR - Estrelas Radiantes", "TEF - Forças Temporais", "SSH - Espada e Escudo", "BUS - Sombras Ardentes", "PHF - Força Fantasma", "RCL - Rixa Rebelde", "NBSP - Nintendo Black Star Promos", "EE9 - Astros Cintilantes"];

export const COLLECTIONS = Array.from(
  new Set([...CARDS.map((c) => c.collection).filter(Boolean), ...EXTRA_COLLECTIONS]),
).sort();

export const FINISHES: Finish[] = ["Normal", "Reverse Foil", "Foil", "Pokebola", "Energia", "Promo", "Ímã"];
