...
function AccessoriesAdmin() {
  ...
  const createNew = async () => {
    const { data, error } = await supabase
      .from("accessories")
      .insert({ title: "Novo acessório", category: CATEGORIES[0], price_cents: 0, stock: 0, images: [], active: false })
      .select("*")
      .single();
    if (error) { toast.error(error.message); return; }
    await load();
    setEditing(data as Accessory);
  };
  ...
  const move = async (id: string, dir: -1 | 1) => {
    ...
    if (!swap) return;
    await supabase.from("accessories").update({ sort_order: swap.sort_order }).eq("id", id);
    await supabase.from("accessories").update({ sort_order: items[idx].sort_order }).eq("id", swap.id);
    await load();
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">Nenhum acessório cadastrado.</p>
        ) : (
...
function AccessoryEditor({ item, onClose, onSaved }: { item: Accessory; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl bg-background p-6 shadow-xl">
        ...
        <h2 className="mb-5 text-lg font-bold">Editar acessório</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest">Categoria</label>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
