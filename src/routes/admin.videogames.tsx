...
export const Route = createFileRoute("/admin/videogames")({
  head: () => ({ meta: [{ title: "Videogames — Admin" }] }),
  component: VideogamesAdmin,
});

const CONDITIONS = ["Novo", "Seminovo", "Usado"] as const;
const DEFAULT_PLATFORMS = ["Nintendo Switch"] as const;
...
function VideogamesAdmin() {
  ...
  const createNew = async () => {
    ...
    await load();
    setEditing(data as Videogame);
  };

  const remove = async (id: string) => {
    if (!confirm("Remover este videogame?")) return;
    await supabase.from("videogames").delete().eq("id", id);
    await load();
  };
  ...
  const move = async (id: string, dir: -1 | 1) => {
    ...
    if (!swap) return;
    await supabase.from("videogames").update({ sort_order: swap.sort_order }).eq("id", id);
    await supabase.from("videogames").update({ sort_order: items[idx].sort_order }).eq("id", swap.id);
    await load();
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">Nenhum videogame cadastrado.</p>
        ) : (
              ...
              <li key={p.id} className="rounded-xl border border-border bg-card p-4 flex flex-wrap items-center gap-4">
                ...
                <div className="flex-1 min-w-[200px]">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {p.platform} · {p.condition} · R$ {(p.price_cents / 100).toFixed(2)} · Estoque: {p.stock} · {p.images.length} foto(s)
                    {!p.active && " · Inativo"}
                  </p>
                </div>
      ...
      {editing && (
        <VideogameEditor
          item={editing}
          knownPlatforms={Array.from(new Set([...DEFAULT_PLATFORMS, ...items.map((i) => i.platform).filter(Boolean)]))}
...
function VideogameEditor({
  ...
  const [region, setRegion] = useState<string>(item.region ?? "");
  const [includesBox, setIncludesBox] = useState<boolean>(item.includes_box);
  ...
  const onUpload = async (file: File) => {
    ...
    try {
      ...
      const path = `videogames/${crypto.randomUUID()}.${ext}`;
  ...
  const save = async () => {
    ...
    if (!platform.trim()) { toast.error("Plataforma obrigatória"); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl bg-background p-6 shadow-xl">
        ...
        <h2 className="mb-5 text-lg font-bold">Editar videogame</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest">Plataforma</label>
              <input
                value={platform}
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest">Condição</label>
              <select value={condition} onChange={(e) => setCondition(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest">Região (opcional)</label>
              <input value={region} onChange={(e) => setRegion(e.target.value)} placeholder="Ex: Americana, Japonesa" className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <label className="flex items-end gap-2 text-sm pb-2">
              <input type="checkbox" checked={includesBox} onChange={(e) => setIncludesBox(e.target.checked)} /> Inclui caixa
            </label>
          </div>
