...
export function VideogameModal({ item, onClose }: Props) {
  const { add } = useCart();
  ...
  const handleAdd = () => {
    ...
    add(
      {
        id: `videogame:${item.id}`,
        cardId: `videogame:${item.id}`,
        name: item.title,
        image: imgs[0],
        collection: "Videogame",
        number: "—",
        finish: item.platform as any,
        ...
        <div className="grid gap-6 p-6 sm:grid-cols-2">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
              ...
              {item.includes_box ? " · Com caixa" : " · Sem caixa"}
            </p>
            {item.description && (
