...
export function PanelModal({ panel, onClose }: Props) {
  const { add } = useCart();
  const [idx, setIdx] = useState(0);
  const [qty, setQty] = useState(1);

  useEffect(() => { setIdx(0); setQty(1); }, [panel?.id]);

  if (!panel) return null;

  const imgs = panel.images.length ? panel.images : [""];
  const inStock = panel.stock > 0;
  const price = panel.price_cents / 100;
        ...
        <div className="grid gap-6 p-6 sm:grid-cols-2">
          <div>
            <div className="relative aspect-square overflow-hidden rounded-lg bg-secondary">
              {imgs[idx] && <img src={imgs[idx]} alt={panel.title} className="h-full w-full object-cover" />}
              {imgs.length > 1 && (
                <>
                  <button
          ...
          <div className="flex flex-col">
            ...
            {panel.description && (
              <p className="mt-2 text-sm text-muted-foreground whitespace-pre-line">{panel.description}</p>
            )}
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {inStock ? `${panel.stock} em estoque` : "Esgotado"}
            </p>
            ...
            {inStock && (
              <div className="mt-4 flex items-center gap-2">
                ...
                <div className="inline-flex items-center rounded-md border border-border">
                  ...
                  <button onClick={() => setQty((q) => Math.min(panel.stock, q + 1))} className="px-2 py-1 text-sm">+</button>
                </div>
              </div>
            )}
